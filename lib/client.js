...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			console.log("[dsh-dracula] apply: saved =", JSON.stringify(saved), "savedValid =", savedValid, "pref =", ctx.theme.getTheme().preference);
			let armed = savedValid;
			let quietTimer;
			const disarm = () => {
				console.log("[dsh-dracula] disarm (quiet period elapsed, pref =", ctx.theme.getTheme().preference + ")");
				armed = false;
			};
			...
			const reassertAgainstAdoption = () => {
				...
				if (pref === "light" || pref === "dark" || pref === DEFAULT_SKIN) {
					...
					console.log("[dsh-dracula] re-asserting saved skin (pref =", pref + ")");
			...
			ctx.on("theme/change", (snapshot) => {
				console.log("[dsh-dracula] theme/change: pref =", snapshot.preference, "armed =", armed);
				syncSkin(snapshot);
